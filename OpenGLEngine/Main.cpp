...
int main()
{
	...
	world.getSystemManager().addSystem<Buoyancy2System>();
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		world.getSystemManager().getSystem<BuoyancySystem>().Update(fixedDeltaTime, 50.0f);
		world.getSystemManager().getSystem<Buoyancy2System>().Update(fixedDeltaTime, 50.0f);
...
void LoadModels(ECSWorld& world)
{
	world.data.assetLoader->StartModelLoading({
		...
		ModelData("Resources/Models/water/water.fbx", {{"waterColour.png"}}),
		ModelData("Resources/Models/nanosuit/nanosuit.obj"),
		ModelData("Resources/Models/supermarine-spitfire/BoatModel.fbx",
			{{"spitfire_d.png"}}),
...
void MakeAFlightSimulator(ECSWorld & world)
{
	auto waterLevel = world.createEntity(); 
	waterLevel.addComponent<ModelComponent>("Resources/Models/water/water.fbx", Vector3(90, 0, 0), Vector3(0, -2, 0));
	waterLevel.addComponent<TransformComponentV2>(Vector3(0, 216, 0), Vector3(3, 120, 3));
	//"Resources/Models/water/water.fbx"
	//Resources/Models/supermarine-spitfire/spitfire.fbx"
	auto boat = world.createEntity();
	boat.addComponent<TransformComponentV2>(Vector3(0.0f, -10.0f, 0.0f), Vector3(1, 1, 1), Vector3(-90, 0.0f, 0.0f));
	boat.addComponent<ModelComponent>("Resources/Models/supermarine-spitfire/BoatModel.fbx", Vector3(0, 0, 0), Vector3(0, 0, 0));
	boat.addComponent<RigidbodyComponent>();
	boat.addComponent<BuoyancyComponent>(5.0f, 1.0f, 3.0f, 10.0f, boat, Vector3(boat.getComponent<TransformComponentV2>().GetPosition().x, boat.getComponent<TransformComponentV2>().GetPosition().y, boat.getComponent<TransformComponentV2>().GetPosition().z - 25));
	boat.addComponent<Buoyancy2Component>(5.0f, 1.0f, 3.0f, 10.0f, boat, Vector3(boat.getComponent<TransformComponentV2>().GetPosition().x, boat.getComponent<TransformComponentV2>().GetPosition().y, boat.getComponent<TransformComponentV2>().GetPosition().z + 25));
	...
	rudder.addComponent<AeroSurfaceComponent>(boat, Vector3(0, 0, 0), Vector3(0, -150, 0));
	rudder.addComponent<AeroControlComponent>(Vector3(-0.01f, 0, 0), Vector3(0.01f, 0, 0), rudderWingPositiveKeys, rudderWingNegetiveKeys);

	auto engine = world.createEntity();
	engine.addComponent<ThrusterComponent>(boat);
	/*
	engine.addComponent<TransformComponentV2>();
	engine.addComponent<AeroSurfaceComponent>(boat, Vector3(0, 0, 0), Vector3(0, 150, 0));
	std::vector<int> sailPositiveKeys = { GLFW_KEY_S, GLFW_KEY_Q };
	std::vector<int> sailNegetiveKeys = { GLFW_KEY_W, GLFW_KEY_E };
	engine.addComponent<AeroControlComponent>(Vector3(0, 0.1f, 0), Vector3(0, -0.1f, 0), sailPositiveKeys, sailNegetiveKeys);
